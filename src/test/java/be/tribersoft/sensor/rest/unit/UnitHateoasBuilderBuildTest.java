...
public class UnitHateoasBuilderBuildTest {
	...
	public void buildsLinksForAUnit() {
		...
		assertThat(links.get(0).getHref()).endsWith("/api/admin/unit/" + ID_1);
	}
	...
	public void buildsLinksForUnits() {
		...
		assertThat(links.size()).isEqualTo(1);
		assertThat(links.get(0).getRel()).isEqualTo(Link.REL_SELF);
		assertThat(links.get(0).getHref()).endsWith("/api/admin/unit");
		...
		assertThat(firstLinks.get(0).getHref()).endsWith("/api/admin/unit/" + ID_1);
		...
		assertThat(secondLinks.get(0).getHref()).endsWith("/api/admin/unit/" + ID_2);
	}
}

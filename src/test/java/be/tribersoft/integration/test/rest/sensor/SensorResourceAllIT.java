...
public class SensorResourceAllIT {
	...
	public void setUp() {
		...
		sensorJpaRepository.save(sensorFactory.create(new SensorMessageImpl(NAME_1, DESCRIPTION)));
		DateFactory.fixateDate(now.plusDays(1));
		sensorJpaRepository.save(sensorFactory.create(new SensorMessageImpl(NAME_2, null)));
	...
	public void getsAllSensors() {
		...
		then(). 
				...
				body("_links.self.href", is("http://localhost:" + port + "/api/sensor")).
				...
				body("_embedded.sensors[0]._links.self.href", is("http://localhost:" + port + "/api/sensor/" + sensors.get(0).getId())).
				...
				body("_embedded.sensors[1]._links.self.href", is("http://localhost:" + port + "/api/sensor/" + sensors.get(1).getId())).
